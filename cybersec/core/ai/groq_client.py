...
class RuleBasedFallback:
    """Rule-based AI analysis without API keys."""
    
    def analyze_scan(self, scan_data: dict) -> dict:
        """Rule-based scan analysis."""
        open_ports = scan_data.get("open_ports", [])
        target = scan_data.get("target", "")
        
        executive_summary = f"Scan of {target} found {len(open_ports)} open ports."
        port_remediations = {}
        ...
        for port in open_ports:
            ...
            if port_num == 22 and service == "ssh":
                port_remediations[str(port_num)] = {
                    "remediation": "Use key-based authentication, disable password auth, keep SSH updated.",
                    "fix_script": "sed -i 's/#PasswordAuthentication yes/PasswordAuthentication no/' /etc/ssh/sshd_config && systemctl restart sshd"
                }
            elif port_num == 80 or port_num == 443:
                port_remediations[str(port_num)] = {
                    "remediation": "Implement HTTPS, use security headers, regular updates.",
                    "fix_script": "ufw allow 80/tcp && ufw allow 443/tcp"
                }
            else:
                port_remediations[str(port_num)] = {
                    "remediation": f"Review necessity of {service} on port {port_num}, consider firewall rules.",
                    "fix_script": f"ufw deny {port_num}/tcp"
                }
        
        return {
            "executive_summary": executive_summary,
            "port_remediations": port_remediations
        }
...
class MultiProviderAIClient:
    ...
    def __init__(self):
        ...
        self._rule_engine = RuleBasedFallback()
        ...
        print("  - Rule-based fallback (no API key required)")
