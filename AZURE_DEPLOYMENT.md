...
3. After creation, get connection string from **Connection strings** tab
4. Update your `DATABASE_URL` in App Service settings

### Option B: Use Railway/External Database

You can use Railway for PostgreSQL and connect to it from Azure.
...
# Or deploy via ZIP
az webapp deployment source config-zip --resource-group cybersec-rg --name cybersec-app --src deployment.zip
```
